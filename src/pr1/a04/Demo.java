...
public class Demo extends AbstractApplication {
    ...
    /**
     * ConsolePrinter und FilePrinter werden ggf. automatisch instantiiert. Nach Ende von run() werden
     * alle Streams geschlossen.
     */
    public void run() {
        ...
        // benutzt Ordnerstruktur nach Package der angegebenen Klasse. Hier: ./data/a03/write_numbers.txt
        ...
        withFileScanner("test.txt", s -> {
            s.useDelimiter(Delimiter.WHITESPACE_OR_COMMA.getPattern()).tokens()
                    .filter(token -> token.matches(MatchPattern.INTEGER.getRegex()))
