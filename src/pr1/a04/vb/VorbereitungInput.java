...
public class VorbereitungInput extends AbstractApplication {
    ...
    public void scannerAusprobieren() {
        ...
        createInputScanner(intNumbers).withInputScanner(s -> {
            ...
            s.tokens().filter(f -> f.matches(MatchPattern.INTEGER.getRegex())).mapToInt(Integer::parseInt).forEach(i -> printf(intFormat, i));
        });
        println();
        ...
        createInputScanner(doubleNumbers).withInputScanner(s -> {
            s.useDelimiter(Delimiter.WHITESPACE.getPattern());
            s.tokens().filter(f -> f.matches(MatchPattern.DOUBLE.getRegex())).mapToDouble(Double::parseDouble).forEach(i -> printf(doubleFormat, i));
        });
        println();
        ...
        // Standardmäßig wird als Pfad die Package-Hierarchie genutzt, daher
        // zuerst zwei
        // Ebenen höher gehen.
        withFileScanner("../../testfiles/zahlen01.txt",
                s -> print(formatScannedIntOrDouble(intFormat, doubleFormat,
                        s)));
    }

    private String formatScannedIntOrDouble(String intFormat,
                                            String doubleFormat, Scanner s) {
