...
public class FirstInput extends AbstractApplication {
    ...
    public void run() {
        ...
        copyNumberFile("../a03/write_mixed_numbers_100.txt", "new_write_mixed_numbers.txt",
                10, 0);
    }
    ...
    public double sumOfNumbersIn(String filename) {
        ...
        withFileScanner(filename, scanner -> sum.add(sumOfNumbersIn(scanner)));
        return sum.getSum();
    }

    /**
     * Übergibt Zahlen vom Scanner an den PrintWriter. Breite und Präzision können manuell eingestellt werden.
     * <p>
     * Für eine automatische Breite sollten {@link SomeFormats#addNumber(double, int)} und
     * {@link SomeFormats#printAllPretty(PrintWriter, int, int)} verwendet werden.
     * <p>
     * Wir benutzen aus Bequemlichkeit die Lösung aus Aufgabe 3, wo wir bereits die hübsche
     * Formatierung umgesetzt haben.
     *
    ...
    public void prettyPrintNumbersFrom(Scanner scanner, int width, int precision, PrintWriter out) {
        ...
        scanner.tokens().filter(f -> f.matches(MatchPattern.NUMBER.getRegex())).forEach(f -> {
            if (f.matches(MatchPattern.INTEGER.getRegex())) {
                SomeFormats.printPretty(out, Integer.parseInt(f), width + precision / 2 + precision, 0);
            } else {
                SomeFormats.printPretty(out, Double.parseDouble(f), width, precision);
            }
            if (counter.get() % 10 == 0) {
                out.println();
            }
    ...
    /**
     * Kopiert alle Zahlen ({@link Integer} und {@link Double}) von der Ursprungsdatei in die Zieldatei und ändert dabei
     * die Breite der Zahl sowie die Präzision der Nachkommastellen.
     * <p>
    ...
    public void copyNumberFile(String originFilename, String destinationFilename, int width, int precision) {
        ...
        withFileScanner(originFilename, scanner ->
                prettyPrintNumbersFrom(scanner, width, precision, getFilePrintWriter()));
    }
