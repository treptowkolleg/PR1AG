...
public class FirstInput extends AbstractApplication {
    ...
    public void run() {
        ...
        withFileScanner(SomeFormats.class, "write_numbers.txt",
                scanner -> prettyPrintNumbersFrom(scanner, 5, 0,
        ...
        withFileScanner(SomeFormats.class, "write_mixed_numbers_100.txt",
                scanner -> prettyPrintNumbersFrom(scanner, 5, 2,
                        getConsolePrintWriter()));

        // Zahlen von A nach B mit neuer Breite kopieren
        copyNumberFile("../a03/write_mixed_numbers_100.txt",
                "new_write_mixed_numbers.txt", 10, 2);
    }
    ...
    /**
     * Übergibt Zahlen vom Scanner an den PrintWriter. Breite und Präzision
     * können
     * manuell eingestellt werden.
     * <p>
     * Für eine automatische Breite sollten
     * {@link SomeFormats#addNumber(double, int)} und
     * {@link SomeFormats#printAllPretty(PrintWriter, int, int)} verwendet
     * werden.
     * <p>
     * Wir benutzen aus Bequemlichkeit die Lösung aus Aufgabe 3, wo wir
     * bereits die
     ...
     * @see SomeFormats
     */
    public void prettyPrintNumbersFrom(Scanner scanner, int width,
                                       int precision, PrintWriter out) {
        ...
        scanner.tokens().filter(f -> f.matches(MatchPattern.NUMBER.getRegex())).forEach(element -> {
            if (element.matches(MatchPattern.INTEGER.getRegex())) {
                SomeFormats.printPretty(out, Integer.parseInt(element),
                        width + precision / 2 + precision, 0);
            } else {
                SomeFormats.printPretty(out, Double.parseDouble(element),
                        width, precision);
            }
            if (counter.get() % 10 == 0) {
                out.println();
     ...
     * Ursprungsdatei in die Zieldatei und ändert dabei die Breite der Zahl
     * sowie
     ...
     */
    public void copyNumberFile(String originFilename,
                               String destinationFilename, int width,
                               int precision) {
        createFileWriter(destinationFilename);
        withFileScanner(originFilename,
                scanner -> prettyPrintNumbersFrom(scanner, width, precision,
                        getFilePrintWriter()));
    }
}
